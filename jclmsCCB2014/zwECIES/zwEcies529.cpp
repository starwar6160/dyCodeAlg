...
#include "zwEcies529.h"
	...
	int ZWOCTET_TO_ASCII(const octet * srcOct, char *dstAscii,
			     const int dstLen);
	void myKDFHMACSeed(octet * KDF2Seed, octet * HmacSeed);
	int ZWOCTET_OUTPUT_STRING(const octet * srcOct, char *dstStr,
				  const int dstLen);
...
//初始化ECIES，并生成私钥和公钥
ZWECIES_API int zwEciesKeyPairGen(const char *password, char *outPriKeyStr,
				  ...
				  const int pubLen)
{
	...
	ECP_KEY_PAIR_GENERATE(&eciesCtx, NULL, &oct_private_key.Value(),
			      &oct_public_key.Value());
	//检验生成的公钥正确性
	int res =
	    ECP_PUBLIC_KEY_VALIDATE(&eciesCtx, TRUE, &oct_public_key.Value());
	if (res != 0) {
...
ZWECIES_API int zwEciesEncrypt(const char *pubkeyStr, const char *PlainText,
			       char *outEncryptedSyncKeyStr,
			       const int syncKeyLen, char *outMsgHashStr,
			       const int hashLen, char *outCryptedTextStr,
			       const int cryptLen)
{
	...
	ECP_ECIES_ENCRYPT(&ecies_ctx, &KDF2Seed.Value(), &HmacSeed.Value(),
			  &RNG, &oct_pubkey.Value(), &oct_PlainText.Value(),
			  oct_hash.Value().max / 2, &oct_syncKey.Value(),
			  &oct_crypt.Value(), &oct_hash.Value());
	//据观察到的现象，非法内容的公钥，会造成HASH和加密结果长度为0
	if (oct_hash.Value().len == 0 || oct_crypt.Value().len == 0) {
		return ECIES_PUBKEY_INVALID;
	}

	if (oct_syncKey.Value().len == 0
	    || ECIES_SUCCESS != ZWOCTET_TO_ASCII(&oct_syncKey.Value(),
						 outEncryptedSyncKeyStr,
						 syncKeyLen)) {
		printf("EncryptedSyncKey Buffer Too Short\n");
		return (ECIES_ENCEDSYNCKEY_TOO_SHORT);
	}
	if (ECIES_SUCCESS !=
	    ZWOCTET_TO_ASCII(&oct_hash.Value(), outMsgHashStr, hashLen)) {
		printf("MsgHash Buffer Too Short\n");
		return (ECIES_HASH_TOO_SHORT);
	}
	if (ECIES_SUCCESS !=
	    ZWOCTET_TO_ASCII(&oct_crypt.Value(), outCryptedTextStr, cryptLen)) {
...
ZWECIES_API int zwEciesDecrypt(const char *prikeyStr, char *outPlainText,
			       const int plainLen,
			       const char *EncryptedSyncKeyStr,
			       const char *MsgHashStr,
			       const char *CryptedTextStr)
{
	...
	myKDFHMACSeed(&KDF2Seed.Value(), &HmacSeed.Value());
	BOOL res =
	    ECP_ECIES_DECRYPT(&ecies_ctx, &KDF2Seed.Value(), &HmacSeed.Value(),
			      &oct_sykey.Value(), &oct_crypted_bin.Value(),
			      &oct_hash.Value(), &oct_prikey.Value(),
			      &oct_plain.Value());
	...
	if (ECIES_SUCCESS !=
	    ZWOCTET_OUTPUT_STRING(&oct_plain.Value(), outPlainText, plainLen)) {
...
int myTestEccMain()
{
	...
	char encSyncKey[ZW_ECIES_ENCSYNCKEY_LEN * 2],
	    msgHashBuf[EFS * 2 + ZW_EXA];
	char cryptText[ZW_ECIES_MESSAGE_MAXLEN * 2];
	if (ECIES_SUCCESS !=
	    zwEciesKeyPairGen("privatekeypassword", PriKey, sizeof(PriKey),
	...
	//////////////////////////////////////////////////////////////////////////
	{
		...
		printf("T(HASH of message)=\t%s\n", msgHashBuf);
		{
			...
			if (!zwEciesDecrypt
			    (PriKey, plainTextBuf, sizeof(plainTextBuf),
...
//自定义实现split函数
void zwsplit(const std::string & s, std::string & delim,
...
static string myMergeEncOutItems(int eciesHandle)
{
	...
	return stu->encoutSyncKey + "." +
	    stu->encoutHash + "." + stu->encoutCryptText;
}
...
ZWECIES_API const char *EciesDecrypt(const char *priKey, const char *cryptText)
{
	...
	int res =
	    zwEciesDecrypt(priKey, g_plainTextBuf_decrypt,
			   ZW_ECIES_MESSAGE_MAXLEN,
			   encout[0].c_str(), encout[1].c_str(),
			   encout[2].c_str());
